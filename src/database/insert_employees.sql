use `Managing-Employee-Made-Simple-Schema`;

INSERT INTO `employee`(`id`, `designation`, `first_name`, `last_name`, `address`, `email`) VALUES 
	(1,'CEO','Magnus','Carlsen','Norway','mc@gmail.com'),
	(2,'MD','Fabiano','Caruana','Usa','fb@gmail.com'),
	(3,'ED','Gary','Kasparov','Russia','gk@gmail.com'),
	(4,'ED','Anatoly','Karpov','Russia','ak@gmail.com'),
	(5,'VP','Sasha','Grischuk','Russia','sg@gmail.com'),
    (6,'VP','Sergey','Karjakin','Russia','sk@gmail.com'),
    (7,'Senior Manager','Danil','Dubov','Russia','dd@gmail.com'),
    (8,'Senior Manager','Dimitry','Andreikin','Russia','da@gmail.com'),
    (9,'Manager','Ian','Nepomniatchi','Russia','in@gmail.com'),
    (10,'Manager','Wang','Hao','China','wh@gmail.com'),
    (11,'Senior Associate','Alireza','Firouja','Iran','af@gmail.com'),
    (12,'Senior Associate','Parham','Maghsoodlu','Iran','pm@gmail.com'),
    (13,'Associate','Yu','Yanghi','China','yy@gmail.com'),
    (14,'Associate','Wesley','So','Usa','ws@gmail.com');
